...
import com.killerappzz.spider.ui.InputXY;
...
public class InputTouchScreen {
	...
	public boolean getTriggered(int index, float time, float frameDelta) {
		boolean triggered = false;
		if (index < MAX_TOUCH_POINTS) {
			triggered = mTouchPoints[index].getTriggered(time, frameDelta);
		}
		return triggered;
	}
	...
	public boolean getTriggered(float gameTime, float frameDelta) {
		boolean triggered = false;
		for (int x = 0; x < MAX_TOUCH_POINTS && !triggered; x++) {
			triggered = mTouchPoints[x].getTriggered(gameTime, frameDelta);
		}
		return triggered;
	}
